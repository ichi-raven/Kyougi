...
void Draw::set_coodinate(const int X_NUM, const int Y_NUM)
{
	...
	int diff_x = X_MIN + (X_MAX / 2) - (TROUT_SIZE * X_NUM / 2);//ðŒ‚É‰ž‚¶‚Ä‘ã“ü
...
void Draw::Draw_line()
{
	...
	{
		DrawLine(Point[0][i].x, Point[0][i].y, Point[y_size - 1][i].x, Point[y_size - 1][i].y, GetColor(255, 255, 255) );//X‘¤•`‰æ
	}


	for (int i = 0; i < y_size; ++i)
	{
		DrawLine(Point[i][0].x, Point[i][0].y, Point[i][x_size - 1].x, Point[i][x_size - 1].y, GetColor(100, 255, 255));//Y‘¤•`‰æ
	}
...
void Draw::Draw_number(int X, int Y, int NUMBER)
{
	...
	DrawFormatString(Point[Y][X].x + diff, Point[Y][X].y + diff, GetColor(255, 255, 255), "%d", NUMBER);
...
void Draw::Draw_color(int X, int Y, int COLOR)
{
	...
	DrawBox(Point[Y][X].x, Point[Y][X].y, Point[Y + 1][X + 1].x, Point[Y + 1][X + 1].y, color, true);
